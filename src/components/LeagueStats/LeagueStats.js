        ...
        const nowFixtures = (
          <div id="leaguestats">
            <ul className="listOfMatches">
              ...
              <button
                onClick={el => this.componentDidMount(el)}
                className="buttonStyle buttonspace"
              >
                Back to the Country List
              </button>
              <button
                onClick={el => this.takeLeagues(el)}
                data-id={FixturesId}
                className="buttonStyle buttonspace"
              >
              ...
              <div className="arrowcontainer">
                <button
                  data-id={FixturesId}
                  onClick={el => this.prevFixtures(el)}
                ...
                <button
                  data-id={FixturesId}
                  onClick={el => this.nextFixtures(el)}
                  className="arrowButton"
                >
                  &#10095;
                </button>
              </div>
              {leagueMatchday.map(match => (
  ...
  nextFixtures = e => {
    ...
    fetch(`https://api.football-data.org/v2/competitions/${e.target.dataset.id}/matches`, {
      ...
      .then(response => {
        ...
        const nowFixtures = (
          <div id="leaguestats">
            <h1 className="neon" data-text="Leagues">
              Leagues
            </h1>
            <ul className="listOfMatches">
              <h2>{response.competition.name}</h2>
      ...
      .then(response => {
        ...
        const nowFixtures = (
          <div id="leaguestats" className="leaguestats">
            <NeonHeading>Leagues</NeonHeading>
            <ul className="listOfMatches">
              <h2>{response.competition.name}</h2>
              <button
                onClick={el => this.componentDidMount(el)}
      ...
      .then(response => {
        ...
        const thisTable = (
          <div id="leaguestats" className="listOfMatches leaguestats">
            <NeonHeading>Leagues</NeonHeading>
            <button onClick={el => this.componentDidMount(el)} className="buttonStyle buttonspace">
              Back to the Country List
            </button>
            <button
              onClick={el => this.takeFixtures(el)}
              className="buttonStyle buttonspace"
              data-id={leagueID}
            >
              Fixtures
